function toggleImageComparison() {
    var comparison_image_wrap = this.parentElement.previousElementSibling,
        ...
        next_text_attr_name =
            'data-text' +
            (comparison_image_wrap.classList.contains(toggle_class)
                ? ''
                : '-toggled'),
        next_text;

    if (comparison_image_wrap === null) return;
...
export default function initImageComparison() {
    ...
    for (i = 0; i < comparison_toggles.length; i++) {
        comparison_image_wrap = previousElementSibling(
            comparison_toggles[i].parentElement,
        );
        ...
        comparison_toggles[i].setAttribute(
            'data-text',
            comparison_toggles[i].innerHTML,
        );
