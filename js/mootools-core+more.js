            ...
            var f =
                !b && !h
                    ...
                    : a.apply(
                          g,
                          b && h
                              ? b.concat(Array.slice(arguments))
                              : b || arguments,
                      );
            return g == e ? f : g;
        };
...
(function () {
    ...
    k.setDocument = function (x) {
        ...
        p.brokenStarGEBTN =
            p.starSelectsClosedQSA =
            p.idGetsName =
            p.brokenMixedCaseQSA =
            p.brokenGEBCN =
            p.brokenCheckedQSA =
            p.brokenEmptyAttributeQSA =
            p.isHTMLDocument =
            p.nativeMatchesSelector =
                false;
        var n, o, y, r, s;
        var t,
            c = 'slick_uniqueid';
        ...
        if (p.isHTMLDocument) {
            ...
            if (A.querySelectorAll) {
                try {
                    ...
                    p.starSelectsClosedQSA =
                        t && !!t.length && t[0].nodeName.charAt(0) == '/';
                } catch (w) {}
                try {
                ...
                try {
                    ...
                    p.brokenEmptyAttributeQSA =
                        A.querySelectorAll('[class*=""]').length != 0;
                } catch (w) {}
            }
            try {
        ...
        p.hasAttribute =
            z && this.isNativeCode(z.hasAttribute)
                ? function (D, C) {
        ...
        p.contains =
            z && this.isNativeCode(z.contains)
                ? function (C, D) {
                      return C.contains(D);
                  }
            ...
            simpleSelectors: if (r) {
                ...
                if (!u) {
                    ...
                    if (u == '#') {
                        ...
                        if (
                            this.idGetsName &&
                            A.getAttributeNode('id').nodeValue != v
                        ) {
                            break simpleSelectors;
                        }
                        if (s) {
            ...
            if (this.isHTMLDocument) {
                getById: if (n) {
                    ...
                    if (
                        ...
                        (this.idGetsName &&
                            v &&
                            v.getAttributeNode('id').nodeValue != n)
                    ) {
                        o = q.all[n];
                        if (!o) {
                            return;
...
(function () {
    ...
    Element.implement({
        setProperty: function (e, l) {
            var D = d[e.toLowerCase()];
            if (D) {
                D(this, l);
            } else {
                if (l == null) {
        ...
        getProperty: function (E) {
            ...
            if (q && !g[E]) {
                var l = this.getAttributeNode(E);
                if (!l || l.expando) {
                    return null;
                }
            }
            var e = Slick.getAttribute(this, E);
            return !e && !Slick.hasAttribute(this, E) ? null : e;
        },
        ...
        getStyle: function (o) {
            ...
            if (Browser.opera || (Browser.ie && isNaN(parseFloat(i)))) {
                if (/^(height|width)$/.test(o)) {
                    ...
                    j.each(function (p) {
                        l +=
                            this.getStyle('border-' + p + '-width').toInt() +
                            this.getStyle('padding-' + p).toInt();
                    }, this);
...
(function () {
    ...
    a.implement({
        ...
        getGMTOffset: function () {
            ...
            return (
                (s > 0 ? '-' : '+') + p((s.abs() / 60).floor(), 2) + p(s % 60, 2)
            );
        },
        setAMPM: function (s) {
            s = s.toUpperCase();
            ...
            return (
                ...
                c.scheme +
                    ...
                    (c.user
                        ...
                        : '') +
                    (c.host || '') +
                    (c.port && c.port != this.schemes[c.scheme]
                        ? ':' + c.port
                        : '') +
                    (c.directory || '/') +
                    (c.file || '') +
    ...
    Form.Request = new Class({
        ...
        makeRequest: function () {
            ...
            this.request = new Request.HTML(
                ...
                failure: function () {
                    a.fireEvent('complete', arguments).fireEvent(
                        'failure',
                        arguments,
                    );
                },
        ...
        this.request = new Request.HTML(
            ...
            success: function (b, g, f, a) {
                ...
                if (d.length == 1) {
                    ...
                    c = new Element('div', { styles: { display: 'none' } }).adopt(
                        d,
                    );
                }
                ...
                if (this.options.useReveal) {
                    c.set('reveal', this.options.revealOptions)
                        .get('reveal')
                        .chain(e);
                    c.reveal();
                } else {
    ...
    validateField: function (j, b) {
        ...
        if (
            ...
            if (d && !this.hasValidator(j, 'warnOnly')) {
                if (f) {
                    j.addClass('validation-passed').removeClass(
                        'validation-failed',
                    );
                    this.fireEvent('elementPass', [j]);
                } else {
                    j.addClass('validation-failed').removeClass(
                        'validation-passed',
                    );
    ...
    [
        'length',
        {
            ...
            test: function (a, b) {
                if (typeOf(b.length) != 'null') {
                    return (
                        a.get('value').length == b.length ||
                        a.get('value').length == 0
                    );
                } else {
                    return true;
                }
    ...
    [
        'validate-match',
        {
            ...
            test: function (b, c) {
                var d = b.get('value');
                var a =
                    document.id(c.matchInput) &&
                    document.id(c.matchInput).get('value');
                return d && a ? d == a : true;
            },
        },
    ],
    ...
    display: function (b, c) {
        ...
        g.each(function (l, k) {
            ...
            if (k != b) {
                ...
                if (
                    a.alwaysHide &&
                    ((l.offsetHeight > 0 && a.height) ||
                        (l.offsetWidth > 0 && a.width))
                ) {
                    j = true;
    ...
    checkDroppables: function () {
        var a = this.droppables
            .filter(function (d, c) {
                ...
                return (
                    b.x > d.left && b.x < d.right && b.y < d.bottom && b.y > d.top
                );
            }, this)
            .getLast();
        if (this.overed != a) {
        ...
        ['request', 'complete', 'cancel', 'success', 'failure', 'exception'].each(
            function (c) {
                ...
                this.reqBinders[a][c] = function () {
                    this['on' + c.capitalize()].apply(
                        this,
                        [a, b].append(arguments),
                    );
                }.bind(this);
    ...
    Array.implement({
        ...
        hsbToRgb: function () {
            ...
            if (this[1] == 0) {
                ...
                var e = Math.round(
                    ((this[2] * (6000 - this[1] * g)) / 600000) * 255,
                );
                var c = Math.round(
    ...
    setHeadSort: function (a) {
        var b = $$(
            !this.head.length
                ...
                : this.head
                      .map(function (c) {
                          return c.getElements(this.options.thSelector)[
                              this.sorted.index
                          ];
                      }, this)
                      .clean(),
